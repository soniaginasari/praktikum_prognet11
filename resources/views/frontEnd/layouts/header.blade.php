                ...
                <div class="col-sm-10">
                    <div class="shop-menu pull-right">
                        <ul class="navbar navbar-expand-lg navbar-light bg-light">
                            ...
                            @php
                                $id = Auth::id();
                                $jum = DB::table('transactions')->where([
                                                                    ['user_id','=',$id],
                                                                    ['status','=',NULL]
                                                                    ])->count();
                                $notif =DB::table('transactions')->where([
                                                                    ['user_id','=',$id],
                                                                    ['status','=',NULL]
                                                                    ])->get();
                            @endphp
                        ...
                        <li class="dropdown">      
                           <a class="dropdown-toggle" data-toggle="dropdown" href="#"> <a href="{{url('/viewcart')}}"><i class="fa fa-shopping-cart"></i> Cart</a>
                         @if($jum != 0)
                         @php
                            $item="anda belum bayar";
                        @endphp
                         <span class="badge" style="background-color: red;">{!!$jum!!}</span>@endif <span class="caret"></span></a>
                         <ul class="dropdown-menu">

                                    <!-- <li ><button id="readnotif" ><a style="color: green;" >Mark All As Read</a></button></li><br> -->
                                    @foreach($notif as $notif)
                                        <li><a href="/transaction/{!!$notif->id!!}">{!!$item!!}</a></li><br>
                                    @endforeach
                                </ul>
                            </li>        
                            @php
